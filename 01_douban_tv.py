...
class DoubanSpider:
    def __init__(self):
        ...
        with open("douban_tv.csv", "a", encoding="GBK") as f:  #设置csv的第一行的数据
            f.write("标题,导演,演员,评分人数,评分"+"\n")
    ...
    def save_content_list(self, content_list,country):  # 保存数据
        with open("douban_tv.csv","a",encoding="GBK") as f:
            for content in content_list:
                ...
                temp_list = [content["title"],"/".join(content["actors"]),"/".join(content["directors"]),content["rating"]["count"],content["rating"]["value"]]
